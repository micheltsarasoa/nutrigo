...
| E2E | `apps/web/e2e/*.spec.ts` | Playwright against the built app served by the API (`node apps/api/src/index.ts`, one SQLite file per project in `apps/web/.e2e-data/`); the playground preview build on `vite preview`. axe injected from `axe-core` | `e2e` |
